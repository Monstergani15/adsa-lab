...
/**
 * Implementation of maximum bipartite matching algorithm. Transcribe the
 * labeling-flipping
 * implementation of augumenting-path algorithm on the adjacency matrix
 * representation of
...
public class MaxBipartite {
    ...
    public MaxBipartite(double adj[][]) {
        ...
        cols = adj[0].length;
        adjacency = new int[rows][cols];
        // convert the given ajdacency matrix of double to an upper triangle adjacency
        // matrix of int.
        for (int i = 0; i < rows; i++) {
    ...
    /**
     * The access point to start the match computation. As mentioned in the
     * algorithm, this
     * algorithm needs to start with some matching. Although empty set is a match,
     * it is
     * desirable to start with a match of bigger size. A simple greedy search is
     * applied to
     * scan row-by-row and find "1" which column id isn't identical to previously
     * selected
     * 1s.
     */
    public void match() {
        // the accumulating set of column IDs of "1" already selected into the initial
        // matching.
    ...
    /**
     * Labeling phase. It's a little tricky to control the flow as meant in the
     * algorithm.
     */
    protected void label() {
        // column scanning
        boolean colScanDone = true;// flow control flag. true = no column exists as labeled but not
        ...
        // row scanning
        boolean rowScanDone = true;// flow control flag, similar to colScanDone.int freeRow = -1;// this number is
                                   // used to control the flow. As described in the algorithm,when a labeled row
                                   // contains no 1*, should exit from this sub and go to flipping(). I call this
                                   // row"freeRow". A value of -1 indicates no free row is found and should
                                   // continue at column scanning.Otherwise, go to flipping().
        for (int i = 0; i < rows; i++) {
            if (rowLabel[i] != NOT_LABELED && !rowScan[i]) {
                ...
                rowScan[i] = true;
                if (!foundMatch) {// no 1* found in this row, should (prematurely) exit from labeling phase and go
                                  // to flipping phase
                    freeRow = i;
                    break;
                }
    ...
    /**
     * Reset the labeling of rows and columns. This is a separate sub because it is
     * called
     * not only at the beginning of labeling, but also before flipping exit back to
     * labeling.
     */
    ...
    /**
     * Return the new adjacency matrix which contains the solution of maximum
     * matching.
    ...
    /**
     * Utility function. Post process the adjacency matrix and return the matching
     * number |M|.
     */
